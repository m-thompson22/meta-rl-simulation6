...
from core.utils import apply_dopamine_manipulation
...
@torch.no_grad()
def unrolled_session_lesion(actor, critic, config, episode=0, block_type="control", lesion_actor=False):
    ...
    for trial in range(n_trials):
        ...
        if is_forced:
            ...
            logits, hx_actor, _ = actor(actor_input, hx_in)
        ...
        scaled_rpe = torch.clamp(rpe, min=-10.0, max=10.0).item()
...
def meta_test_lesion(actor, critic, config, lesion_actor=False, reset_actor_each_episode=False):
    ...
    for condition in conditions:
        ...
        for episode in range(config.num_episodes_test):
            ...
            trial_history = unrolled_session_lesion(
                actor=actor,
                critic=critic,
                config=config,
                episode=episode,
                block_type=condition,
                lesion_actor=lesion_actor
            )
            ...
            results['rewards'].append(trial_history['rewards'])
            results['arms'].append(trial_history['arms'])
            results['actions'].append(trial_history['actions'])
            results['p_riskys'].append(trial_history['p_riskys'])
            results['entropies'].append(trial_history['entropies'])
            results['rpes'].append(trial_history.get('rpes', []))
            results['policy_p_risky'].append(trial_history.get('policy_p_risky', []))
            results['arm_map'].append(trial_history['arm_map'])
            results['safe_action'].append(trial_history['safe_action'])
            results['risky_action'].append(trial_history['risky_action'])
            results['switch_trials'].append(trial_history['switch_trials'])
            results['coefs'].append(coef)
